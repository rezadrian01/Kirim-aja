...
CREATE TABLE `shipment_details` (
    ...
    `recepient_name` VARCHAR(191) NOT NULL,
    `recepient_phone` VARCHAR(191) NOT NULL,
...
-- CreateTable
CREATE TABLE `_ShipmentToUser` (
    `A` INTEGER NOT NULL,
    `B` INTEGER NOT NULL,

    UNIQUE INDEX `_ShipmentToUser_AB_unique`(`A`, `B`),
    INDEX `_ShipmentToUser_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `_ShipmentToUser` ADD CONSTRAINT `_ShipmentToUser_A_fkey` FOREIGN KEY (`A`) REFERENCES `shipments`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_ShipmentToUser` ADD CONSTRAINT `_ShipmentToUser_B_fkey` FOREIGN KEY (`B`) REFERENCES `users`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
